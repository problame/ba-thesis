...
%Over the last decade, internet giants like Amazon, Google \& Facebook have demonstrated the advantages of \glslink{horizontally scalable}{scale-out} application architectures, pioneering the \gls{container} movement and the \glspl{microservice} design pattern.

% With the rise of cloud computing  \gls{iaas} platforms

% is a manifestation of  encourages highly specialized component services that are assembled into individual applications 
% horizontally scaled applications (\textit{scale out} workloads) have become 
% The software industry 
% \cite{kanev2015profiling}
% Horizontally scaled workloads

% TODO blablabla about how much the cloud is important

% sw industry trans to cloud
% smartphones / low power / resource devices
% outsourced to cloud service providers (SaaS) over the internet
% thus, server apps are important
...
% The following heuristics have proven useful for finding stages in application code:
% \begin{itemize}
%     \item high-level application library calls (de/encoding, input validation, backend RPC)
%     \item stack-sampling \& subdividing into stages proportional to the relative amount of CPU time spent in each part of the request 

% Each IO and in particular the \stage{proc} stage are certain to access a significant amount of text and data memory:
% in total, the \gls{working set} size of the request handler will exceed the on-core-cache size on  \cite{kanev2015profiling}.
...
% explain limitations of commodity hardware on which applications are deployed today
% Such a service will most likely be part of a larger application where
% individual services are designed to be horizontally scalable on demand (see microservices). %TODO ref

% In such scenarios, manually managed OS installations are rare.
% Instead, a service instance is deployed to a dedicated virtual or even physical machine. %TODO ref

% This approach of single-purpose machines is taken to the extreme by \glspl{libos} (\textit{unikernels}):
% the entire virtual machine image only consists of those pieces of code required by the application,
% promising both performance and security improvements. % TODO ref
% % whole system optimiazations!
%By its nature, this VM will only run the code required to fulfill the  the application code  single-purpose and thus can be radically optimized for the application.
%which only contains the code required(see \textit{Background and Related Work}).% TODO ref
...
%leveraging knowledge about its current stage, a thread is scheduled on a processor that executed the same stage before.

% A first proof-of-concept implementation in Linux shows more than $10\%$ performance gains %TODO ref
% for the MySQL server. However, TODO what is the exact problem with the approach taken in linux?
...
% Subsequently, the policy shall be evaluated using performance benchmarks comparing our implementation to
% both unmodified \gls{osv} and an up-to-date Linux kernel.
% why...

% Building on a proof-of-concept by \citeauthor{gottschlag2017} implemented in the Linux,
% this theses' goal is
...
% Operating system threads (traditionally called kernel threads) take precedence over any threads of $A$ 
% and may define their own stages. %TODO ? then use a new variable for S below
...
% to keep in mind when designing the scheduling policy  proposes a very basic policy and raises some concerns
% picked up again in the \textit{Evaluation} chapter \textit{below}: % TODO ref
