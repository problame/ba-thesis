...
We present an application of \emph{staged and event-driven architecture} (SEDA) in a research database manamgement system and the resulting performance benefits due to reduced i-cache misses and branch prediction.
...
We show that our work is relevant for contemporary applications by presenting profiling results showing that large i-cache footprint is responsible for CPU pipeline stalls in contemporary scale-out applications.
