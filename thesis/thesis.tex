...
We conclude with the starting point for this thesis: a proof-of-concept implementation at the KIT OS group which extends computation spreading to arbitrary split points within user-level code.

\NewDocumentCommand{\relworksection}{m m g}{
    \section{#1}\label{#2}
    \IfValueT{#3}{%
        \sectionsubheading{This section summarizes \cite{#3} by \citeauthor*{#3}.}
    }%
}

\relworksection{Concurrency Models in the Early 2000s}{ch:relwork:conmod}
...
\relworksection{Memory Hierarchy on Contemporary Server-Class CPUs}{ch:relwork:memhier}
...
The authors do not provide explicit reasons for the abnormal size of the instruction working sets, only mentioning \textquote{lukewarm} code and static linking as possible contributors.
...
\relworksection{Adoption of staged computation}{ch:relwork:adoption}
...
\relworksection{Cohort Scheduling on SMP Systems}{ch:relwork:cohortsmp}
