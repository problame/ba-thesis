...
Motivated by recent profiling work at Google datacenters which proves that scale-out applications' large instruction cache footprint is still cause for major performance bottlenecks on contemporary CPUs,
...
A solution to use the memory hierarchy on SMP systems more effectively has already been published under the name \emph{computation spreading}:
this approach executes OS code on different CPU cores than user-level code by migrating threads on synchronous kernel entry, effectively spreading the instruction working set of OS-intensive applications.
We conclude with a proof-of-concept implementation at the KIT OS group which implements computation spreading to arbitrary separation points in the application code.
...
{\footnotesize This section summarizes \cite{compspr}.} % TODO should become a sub-heading
