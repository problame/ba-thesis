...
Furthermore, since the amortization only reduces the execution time but does not eliminate it, the minimum response time is necessarily increased.
...
Cohort scheduling at the syscall boundary is non-invasive with regards to application code bases and will we revisited by \emph{computation spreading} (Section~\ref{ch:relwork:compsr}).
...
Although the first thread executing this step will incur capacity i-cache misses, it pre-warms i-cache and branch predictors for the remaining nine threads.
...
Lastly, it should be noted that STEPS does not cover any code paths in the operating system, because stray threads are removed from their team.
However, operating system code has a significant cache footprint and thus a potential target for i-cache footprint optimization.~\cite{compspr}
The situation is worsened by the fact that the stray threads will continue to compete for CPU time while they are not blocked.
Since the OS scheduler is unaware of the team lists, it may schedule a stray threads in the midst of a team, evicting its cache state inadvertently.
STEPS tries to prevent this by giving a \textquote{hint} to the scheduler to prioritize the next thread that remained in the team.
We argue that this kind of information loss cannot be fully compensated by scheduler hints, which also imply interaction with the kernel and thus additionaly overhead.~\cite{steps}

\relworksection{Interim: Applicability to SMP Systems}{ch:relwork:cohortsmp}
The related work we presented so far was evaluated on single-core machines.
Adaption to multi-core systems is not mentioned at all or stays theoretical.~\cite{steps,harizopoulos2003case}
We argue that the designs of cohort scheduling and STEPS must be re-examined given the ubiquity of multi-processor systems.
...
The single-core machines used in the approaches above have a small L1 cache and a larger, higher-latency L2 cache.
However, contemporary symmetric multi-processors feature \emph{private} L1 and L2 caches and a \emph{shared} L3 cache.
Access latency between L1, L2 and L3 increases by a factor of 3 -- 4 times between each level.
Out-of-order processing is typically able to mitigate L1 instruction misses but cannot hide L2 misses.\cite{hennessy2002,haswellCacheLatency} %hennessy print page 509

An adaption of cohort scheduling and STEPS to multiprocessors must thus target the private caches to achieve similar benefits.
...
However, in contrast to the approach of \emph{computation spreading}, this replicates to each core the repeated capacity misses when a team moves to the next step.
...
Despite the very specific implementation, the concept presented by the authors is more general and not limited to the syscall interface:
they suggest migration points at arbitrary positions in the application code and corresponding core allocation policies.

\todo{hardware-based approaches?} % http://ieeexplore.ieee.org/document/1410076/

\relworksection{Prototype in Linux at the KIT OS Group}{ch:relwork:kitpoc}
