...
With the Stage API and both synchronous and remote asynchronous thread migration mechanisms in place, we will now present the core allocation policy.
...
After computing a core allocation, we use it for a configurable amount of time \todo {code or revert to reality} to select a target CPUs on stage switch and wake-ups.
...
After an assignment's maximum age has been reached, we compute a new assignment using new stage load values.
...
When a thread synchronously switches stages, we need to find a target CPU that is assigned to the switched-to stage.
...
Additionally, all cores will use the same core allocation concurrently, but our design also involves periodic updating.
The update mechanism must therefore be designed to allow all cores to make progress during an update.
Lastly, an update should be minimal in the number of coures that switch to another stage in order to minimize the sum of capacity misses incured due to the upgrade.
...
On a system with $C$ cores and $S$ stages, a requirement is a vector ${r = \begin{smallmatrix}( r_1 & r_2 & \dots & r_S )\end{smallmatrix}}$ where ${r_i \in \mathbb{N}_0$ and $\sum_{i = 1}^{S} r_i  = C}$.
An assignment ${a = \begin{smallmatrix}( a_1 & a_2 & \dots & a_C )\end{smallmatrix}}$ then maps all $C$ cores to stages, under the constraint ${\forall s \in 1 \dots S : \vert\{\left i : a_i = s \right \} \vert = r_s}$.
When computing a new assignment $a'$, our goal must then be to minimize ${\vert\{\left i : a_i' \ne a_i \right\}\vert}$ because this is the number of cores that will incur capacity i-cache misses due to the core switching to a new stage.
...
We compute the new $r'$ as follows:
...
The minimal transition is then implemented by building a delta vector ${\Delta r = r' - r}$:
...
We use RCU for concurrent use and update of the assignment:
