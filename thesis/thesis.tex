...
A proof-of-concept implementation in Linux and the MySQL database management system shows that i-cache misses and branch-misprediction rates can be reduced by X and Y\%.
However, experiments with multiple concurrent requests show that the prototype is not work-conserving on multi-core systems.\todo{data}
...
In Chapter~\ref{ch:relwork}, we examine preceding work in the area of staged computation, including an analysis of their applicability to multi-core  systems.
...
The expected reductions of i-cache and mispredicted branches are achieved in the modified version, but the speedup in throughput is dependent on the number of concurrent requests:
Taking upstream as baseline (100\%), throughput with a single TPC-C terminal is 8.6\% higher in the modified version.
However, for multiple terminals and thus multiple concurrent clients, throughput only increases up to a factor of the number of stages.~\todo{check}
...
Second, the patches are specific to the simulated cache hierarchy, requiring ready-to-run images to be built per targeted CPU.
...
For an increased number of terminals, we observe that our solution maintains an 8 -- 10\% speedup with MWAIT, and gains another 10 percent points when using busy-waiting.
