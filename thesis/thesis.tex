...
And naturally, the thread migration should be minimally disruptive to both CPUs' cache and execution state.
...
Our solution thus uses the \textsc{\texttt{monitor}} and \textsc{\texttt{mwait}} instructions to get notified about changes to the incoming migrations bitmask while keeping the idle core in a low-power state. \todo{dark silicon advantage over busy-waiting, put in eval / discussion?}
...
While hyperthreading-support in the core allocation policy is subject to future work, \todo{check this is in} the choice to use \textsc{\texttt{mwait}} is therefore a necessary step towards that goal.


\section{Work Conservation on Multi-Core Systems}\label{ch:di:wake}
...
Utilizing all cores in of a multi-core system is thus trivial in theory:
When a blocked thread is woken up, we simply find a CPU that has that thread's current stage in its cache and enqueue the woken-up thread's TCB into that CPU's runqueue.

However, in practice, this beautiful, simple concept is crushed by the realities of the OSv scheduler implementation.
...
The most relevant observation in Figure~\ref{fig:di:wake:upstreamstates} is that the upstream OSv thread states do not encode whether the thread has finished scheduling out.
...
In each of these states, the thread can either have completed scheduling out or it can still be in the midst of doing this.
In the latter case, it may even observe that it has already been woken up again and move itself from \textit{waking} back to \textit{running}.
...
If the thread is still running, we cannot migrate it from the context of the asynchronous waker and we do not want to migrate it because its current CPU still has a hot cache.
If however the thread has stopped running, we want to migrate it to a less loaded core assigned to the same stage in order to be work conserving.
And in contrast to a running thread, a stopped thread can be migrated using OSv's built-in incoming wakeup queues as described in Section~\ref{ch:di:osv}.
...
With the refactoring of the thread states done, there still remains one obstacle for performing migration of woken-up threads in the context of the waker:
As explained in Section~\ref{ch:di:osv:sched}, upstream OSv provides a facility for \emph{remote asynchronous thread migration} which is implemented using a short-lived helper thread to suspend the timers of the to-be-migrated thread.
The helper thread is necessary because the OSv's timer abstraction mandates that a thread's timers be all local to a single core.
Looking at the version control history of upstream OSv, the timer implementation predates support for remote asynchronous thread migration, leading us to believe that the helper thread was added as an afterthought.
Given that upstream only required it for the POSIX \lstinline[style=figurecpp]{pthread_attr_setaffinity_np} API, this is a very understandable and a quite elegant engineering trade-off.
However, our solution requires remote asynchronous thread migration to be \emph{fast}.
Thus, we do neither want to pay the price of spawning a helper thread nor do we want to incur the cost of a remote function call from waker to source CPU to reprogram the its LAPIC-based timer.
...
\end{enumerate}
We observe that the timers in step 1 only need to be \emph{removed} remotely--- resuming them on CPU $C_3$ is the job of its CPU-local idle thread.
We recall that the CPU-local timer lists are sorted ascending by expiration date and that the underlying hardware timer is programmed to the expiration date closest in the future.
...
After the above refactorings, we finally have an efficient way to perform remote asynchronous thread migration in the waker, making our scheduler work conserving
...
With the Stage API and both synchronous and remote asynchronous asynchronous thread migration mechanisms in place, we will now present the core allocation policy.
Remember that the basic idea of our solution is to dedicate CPU cores and hence their private caches to stages, and only dispatch threads onto those cores that are assigned to the thread's current stage.
However, in order to fully utilize the available CPU cores, we cannot assume this assignment to be fixed:
A thread may spend a different amount of time per stage, either because their code has different, fixed runtime or because its runtime is dependent on request parameters.
Therefore, the core allocation must be periodically re-asssessed, and the system must support a changing core allocation efficiently.
...
We use a given core allocation for a configurable duration after which we re-compute a new allocation.
...
In the long-term, load differences between all stages are thus evened out. \todo{why desirable? queuing theory?}
...
For the implementation, let us first consider where we need to consult the core allocation:
At first, when a thread synchronously switches stages, we need to find a target CPU that is assigned to the switched-to stage.
Secondly, when waking up a stopped thread, we need to find a CPU that is assigned to that thread's current stage.
While stage switches may be relatively infrequent throughout the handling of a single request, that is not true for wakeups, for example because the request handler performs blocking IO.
Consulting the core allocation should therefore be fast.
Additionally, all cores will use the same core allocation concurrently, but we also need to implement periodic updating.
The update mechanism must thus be designed such that all cores can make progress, even during an update.
Lastly, an update should be \emph{minimal}, i.e., only the minimally required number of cores should switch to a new stage in order to minimize the capcaity misses of their private i-caches.

Our solution is based around a data structure, which represents a core assignment of CPU cores to stages, and the requirement vector, which only specify the number of cores to be assigned to each stage.
On a system with $C$ CPU cores and $S$ stages, a requirement is a vector $r = \begin{smallmatrix}( r_1 & r_2 & \dots & r_S )\end{smallmatrix}$ where $r_i \in \mathbb{N}_0$ and $\sum_{i = 1}^{S} r_i  = C$.
An assignment $a = \begin{smallmatrix}( a_1 & a_2 & \dots & a_C )\end{smallmatrix}$ then maps the $C$ cores to one stage each under the constraint $\forall s \in 1 \dots S : \vert\{\left i : a_i = s \right \} \vert = r_s$.
When computing a new assignment $a'$, our goal is to minimize $\vert\{\left i : a_i' \ne a_i \right\}\vert$ because this is the number of cores that will incur capacity i-cache misses due to a changed instruction working set.

The initial assignment in the system distributes the available cores using simple modulo arithmetic.
The periodic update then consists of the following two steps:
At first, we compute a new requirement vector $r'$ that distributes CPUs proportional to the stage load.
Afterwards, we compute a new new assignment from the existing assignment and $r'$.

We compute the new requirement $r'$ as follows:
...
The minimal transition is then implemented by building a delta vector ${\Delta r = r' - r}$:  negative comonents represent stages with excess CPU cores and positive components represent need for CPU cores.
A simple nested loop then transfers CPUs between components and updates the assignment accordingly.
Note that the constraints on requirement and assignment vectors guarantee that $\sum_{i = 1}^{S} \delta r_i = 0$, thus guaranteeing a transition is possible.
The asymptotic complexity of loop is in $O(S^2)$, but since we do not expect more than ten stages per application, this is neglegible.

We use RCU to update the assignment periodically:
When accessing the current assignment through its public getter function, we compare the timestamp of the assignment's creation against the current time. \todo{code or revert to reality}
...
RCU thus allows all other threads to safely use the old assignment until the updater atomically replaces it.
