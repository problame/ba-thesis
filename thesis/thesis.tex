...
which can be attributed to both the use of a more expressive programming language and the comfort of only providing device drivers for typical hypversior-emulated devices and paravirtualization.
...
To avoid build and linker failures, we provide a no-op implementation of the API as a shared library.
This enables us to build application binaries that work on both Linux and OSv using the application provided build system.%
...
Let us recall from Section~\ref{ch:di:osv:sched} that
\begin{enumerate}[label=(\alph*)]
    \item the waker may run on another CPU than the woken-up thread,
    \item upstream OSv requires threads to have all their timers on one CPU core and
    \item upstream OSv's remote asynchronous thread migration relies on a short-lived helper thread chasing the to-be-migrated thread until it effectively performs synchronous thread migration.
\end{enumerate}
The helper thread is necessary because of the timer implementation's requirement to keep all of a thread's timers on a single core.
Looking at the version control history of upstream OSv, the timer implementation predates support for remote asynchronous thread migration.
Given that upstream only required it for the POSIX \lstinline[style=figurecpp]{pthread_attr_setaffinity_np} API, we assume that the helper-thread-based solution was considered good enough --- in fact it is a quite elegant engineering trade-off.
However, our solution requires remote asynchronous thread migration to be \emph{fast}.
Thus, we neither want to pay the price of spawning a helper thread nor do we want to incur the cost of a remote function call from waker to source CPU to reprogram the its LAPIC-based timer.
...
At first, we stagify MySQL, a widely-used open-source database management system, and measure its performance using an industry-standard benchmark, showing that our technique yields to X\% higher throughput compared to upstream OSv. \todo{data}
