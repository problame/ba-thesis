...
In this thesis, we analzye existing approaches to optimize the cache behavior of network servers.
...
%In contrast to previous approaches in the field, the API allows spreading the working set at arbitrary positions in the application code.
...
The schema described above leads to a step-by-step progression of the entire team that is depicated in Figure~\ref{fig:cohort_and_steps}.
...
\citeauthor{yasinTopdown}~\cite{yasinTopdown} further simplify this view by only differentiating betwee \emph{frontend} and \emph{backend}:
...
The single trust domain property furthermore allows omitting all privilegelevel switching between OSv and the application.
...
Upstream in contrast is much less affected with only 10\% higher throghput at 10Hz PAS frequency.
