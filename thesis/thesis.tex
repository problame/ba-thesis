...
For example, when choosing 10Hz instead of 1000Hz as the PAS frequency, each invocation of the PAS can in theory run up to 100 times longer, but since the entire page cache may be scannable in a shorter time, the PAS may go back to sleep early.
...
However, the PAS is an exception in this case because it runs for up to 20\% of the CPU time given to it by the scheduler.
Whenever the core allocation policy assigns cpu0 to a stage, that stage's threads will --- invisible to the core allocation policy --- compete with the PAS for CPU-time.
...
The PAS thread's instruction working set is disjoint from the stage's threads' working set and given that the latter is chosen to just fit the CPU-private cache, the frequent scheduling of the PAS thread causes i-cache misses in the private cache of cpu0.
...
We can quantify the effect outlined above by comparing throughput and cache behavior with 1000Hz vs 10Hz PAS frequency.
The results are shown in Figure~\ref{fig:ch:di:osvpageaccessscannerbench}:
With PAS frequency reduced to 10Hz, our solution achieves 17\% higher throughput compared to 1000Hz.
Upstream in contrast is much less affected with only 10\% higher throghput at 10Hz PAS frequency.
The differences in throughput are visible in the respective code miss ratios:
