...
On the source CPU, after $T$ has scheduled out, we then set $T$'s state to \lstinline[style=figurecpp]{statemig_sto}.
...
Also remember that the \textbf{principal rule of our scheduler is to only dispatch threads on cores that are allocated to the thread's current stage}.
...
\textbf{We define \emph{stage load} as the number of \emph{runnable} threads in a given stage and dedicate CPU cores to stages proportional to stages' load.}
...
Whereas stage switches may be relatively infrequent throughout the handling of a single request, that is not true for wakeups, for example because the request handler performs blocking I/O.
...
When requirements change to $r'$ and we need to update the assignment to $a_{r'}'$, our goal must then be to minimize ${\vert\{\left i : a_i' \ne a_i \right\}\vert}$ because this is the number of cores that incurs capacity i-cache misses due to change of the executed instruction working set partition.
...
We compute the requirements vector proportional to the current stage load distribution.
Since we can only assign CPUs to stages in integer quantities, we only use the floored values of the exact proportional distribution.
The remainders vector is then normalized and used as the priority vector for the next iteration of the loop.
It can happen that no stage has enough priority to win at least one CPU.
In those cases, we shift priority from the low-priority stage to the highest-priority stage to ensure that we make progress.
...
\begin{algorithmic}[1]
    ...
    \makebox[\widthof{\textbf{Require:}}][l]{\textbf{Ensure:}}}
      ...
      \Ensure      \begin{varwidth}[t]{\linewidth}
        \Statex $r$, valid requiremnts vector
          \strut
               \end{varwidth}

    \State $\hat{l} \gets norm(l)$ \Comment normalized load vector
    \State ${p \gets \hat{l}}$ \Comment priority vector
    \State $c \gets C$ \Comment number of cores left to assign
    \State $r \gets \begin{pmatrix}0 & 0 & \dots & 0\end{pmatrix}$
    \While{c > 0}
        \State \textbf{assert} ${\sum_{i = 1}^{S} p_i = 1.0}$
        ...
        \EndIf
        \State ${r \gets r + \floor{f}}$ \Comment can only assign integer quantities
        \State ${p \gets norm(f - \floor{f})}$ \Comment priority based on remainders
        \State ${c \gets c - \sum_{i = 1}^{S} \floor{f}_i}$
    \EndWhile
\end{algorithmic}
\end{minipage}
\caption{
...
However, we did not spend time on searching lower-runtime algorithms and we do expect most applications to have $S \le 8$ and $C \le 32$, which interpolates to $\approx 100us$ runtime, based on the results of the evaluation. \todo{ref evaluation} % (8^2)×(32^2) * (512ns/(3^2 * 6^2))
...
The PAS consists of an infinite loop that self-tunes the CPU time it consumes through a target value $t \in [0.1 and 20]$:
...
Whenever the core allocation policy assign CPU0 to a stage, that stage's threads compete with the PAS for CPU-time.
...
To demonstrate the effects described above, we run a single-terminal TPC-C benchmark againt stagified MySQL using our core allocation policy and compare throughput and L2 metrics at PAS frequencies of 1000Hz and 10Hz.
At 1000Hz, we achieve a mean value of 150 requests/sec whereas at 10Hz, we achieve 220 requests/sec (46\% speedup).
Also, we see 30\% more L2 cache misses per request at 1000Hz, 20\% thereof being code read misses.
Upstream OSv in contrast is not as significantly affected by change of the PAS frequency, achieving mean values of 180 vs. ~225 requests/sec, respectively (25\% speedup).

We have experimented with several approaches to solve the problems of the PAS in our solution:
As a first suggestion, we could model CPU0 as a \emph{50 -- 80\% CPU} in our core allocation policy.
...
In the core allocation policy presented in Section~\ref{ch:di:pol}, we dispatch threads in underloaded stages round-robin over all cores. \todo{assert this is in}
...
We evaluate our solution on a single-socket Intel Xeon E5-2618L v3 CPU with 8 cores, 2 hyperthreads each.
The system has 32 GiB of memory consisting of $4*8$ GiB DDR4 DIMMS clocked at 2133 Mhz frequency.
...
All private caches are 8-way set-associative. \todo{need quote?}
Via the BIOS, we disable hyperthreading because hyperthreads share the private caches and disable all C-states to reduce latency spikes.

The host operating system is Fedora 27 with Linux kernel version 4.13.9 (Fedora package \lstinline[style=figurecpp]{4.13.9-300.fc27.x86_64}).
For the evaluation, we use the kernel command line parameter \lstinline[style=figurecpp]{isolcpus=2-7} to isolate cores 2 -- 7 from the Linux scheduler.
We remove the noise caused by frequency scaling by setting the scaling governor to \lstinline[style=figurecpp]{performance} mode and disabling the \lstinline[style=figurecpp]{turbo} frequency range.

Due to the nature of OSv targeting only virtual machines, our evaluation is primarily done using VMs.
...
Each VM is assigned 6 virtual CPU cores, and each of the corresponding QEMU threads is pinned to a separate core in range 2 -- 7.
The remaining QEMU threads which are used for timers, async-I/O, etc., are regularly dispatched by the Linux scheduler on the idle cores 0 and 1.
We run \emph{all} virtual machines in above configuration, and therefore do not compromise the fairness of comparisons made with regards to cache behavior and performance.
Furthermore, our implementation would not be able to take any action against the QEMU helper threads because it exclusively runs inside the virtual machine.
However, this issue makes it very clear that real-world deployments of our solution will require further adjustments to the hypervisor configuration.
\todo{move to limits / future work?}

When quoting benchmarks on \emph{bare-metal} Linux, we refer to measurements conducted directly on the host system.
We use Docker 1.13 to run the official MySQL 5.6.38 container and use the corresponding command line arguments to execute all MySQL threads on CPUs 2 -- 7.
The MySQL data directory is bind-mounted from a location in tmpfs.

For evaluating MySQL, we use the industry-standard TPC-C online transaction processing benchmark.
Specifically, we use the implementation provided by the open source \emph{OLTP Benchmark} project at Git revision \texttt{019d9cf} from Februrary 2018~\cite{oltpbench}.
To avoid the time-consuming pre-seeding process of the database, we re-use a one-time-generated pre-seed with two TPC-C warehouses. \todo{mention weights?}
The appendix \todo{appendix} contains the exact benchmark configuration and the preseed for all TPC-C results presented in this chapter.

We run the OLTP benchmark from a separate machine (\emph{benchmark machine}) connected to the main machine using direct 10 Gbit/s ethernet.
...
For bare-metal benchmarks, we also assign an IP address to the the bridge interface on the main machine's host system and insert a wildcard accept rule in the relevant iptables chains.
...
We rely on perf's time-multiplexing to record more event types than physical PMUs available on the CPU.~\cite{perfTimeMultiplexing}

Finally, for running orchestrating and scripting the execution of the VM, the OLTP benchmark and perf, and for collecting the benchmark results, we use a self-written benchmarking tool called \texttt{doltp} which can be found in the appendix. \todo{appendix}
...
the existing MySQL code-base is large and thus not trivially refactorable to the application architectures presented in Chapter~\ref{ch:relwork} and
the threading model is compatible with our solution.
Furthermore, we will show in the subsequent paragraph that MySQL's can benefit from spreading its instruction working set over multiple cores.
Lastly, databases are still often operated and performance-tuned by database administrators where performance gains through our solution may be tolerable despite the arguably less conventional deployment as a bundled libary OS VM image. \todo{leave that out?} \todo{proof}

We first analyze MySQL performance on bare-metal Linux using the TPC-C benchmark and the Top-Down performance analysis method developed by \citeauthor*{yasinTopdown}~\cite{yasinTopdown} which was also used for large-scale profiling of datacenter applications at Google by \citeauthor*{kanev2015profiling} (see Section~\ref{ch:relwork:profiling}):
55\% of pipeline slots during a TPC-C benchmark run are frontend-bound, 20\% backend-bound and 5\% lost to bad speculation, leaving merely 20\% of pipeline slots to be actually retired. % topdown_comparison.png
\todo{intro to top-down?}
Looking at the the cache behavior, more than 66\% of private L2-cache misses are due to code reads, further supporting the assumption that a MySQL's instruction working set does not fit into the private cache of a single core.
...
As a result, we used the RAII-style switching to separate out
\begin{itemize}
...
\end{itemize}
The above process was very time-consuming because we were not famliar with the MySQL code base and the resulting throughput measured by TPC-C was too unstable to be worth pursuing the approach any further.
...
While the corresponding publication will not be available until April 2018, we can summarize the approach:
Based on a memory access recorded while running a representative workload, different positions for a given number of stage-switching points are evaluated using a cache-simulator.
A human then manually extends the source code to include the desired switching points. \todo{cite}
We used above approach to find an optimal partitioning for three stages. \todo{reason}

The resulting diff statistics are printed in Figure~\ref{fig:eval:api:diffstat} and the full diff can be obtained from the appendix: \todo{appendix}
It amounts to 54 insertions and two deletions throughout nine files, and 32 lines of these insertions are isolated in two new files prefixed with \texttt{my\_osv} that contain the declaration and definition of stage pointers as global variables.
...
While this is prohibitive for fragmented setups, we expect the targeted medium to large-scale deployments to use homogenous hardware, facilitating deployment.
