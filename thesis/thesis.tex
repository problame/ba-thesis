...
\caption{The effect of 10Hz vs. 1000Hz PAS frequency on TPC-C throughput and L2 code misses.
    ...
    The results were produced with enabled MWAIT in the idle thread and 12 TPC-C terminals.
}
...
\begin{figure}
    ...
    \caption{
        ...
        The true frontend-boundedness is only visible when removing any spinning, which we only did in Upstream OSv.
        The break-down is similar to the observations made by \citeauthor*{kanev2015profiling} (Section~\ref{ch:relwork:profiling}).
    }
...
\begin{figure}
    ...
    \caption{
        ...
        More than 66\% of all L2 cache misses are due to code reads, confirming the assumption that MySQL's instruction working set does not fit into the private cache of contemporary processor cores.
    }
...
\begin{figure}
    ...
    \caption{
        ...
        We observe an up to 22\% speedup with our solution with busy-waiting and a 9\% speedup when using MWAIT in the idle thread.
        The \lstinline[style=figurecpp]{max_assignment_age} parameter (left item in parentheses for our solution) does not significantly affect the performance of our solution.
    }
...
Although the above results show that our solution achieves up to 22\% higher throughput in the TPC-C benchmark,
...
Our explanation for this observation is that calling HALT in the VM causes an exit to the hypervisor and thus additional code misses.
...
For the top-down top-level breakdown, we compare the MWAIT versions of upstream and our solution, which is necessary because spinning blows up the \textit{retiring} fraction because a tight loop is neither frontend nor backend-bound.
...
The mean value of 1660ns with MWAIT is comparable to the bare-metal IPI propagation of 1700ns measured in~\cite{ipiLatencyPopcornLinux}, which is exceeded by more than 4800ns by upstream OSv.
...
\begin{figure}
    ...
    \caption{
        ...
        Our solution shows more than 70\% lower migration latency and is less affected by halting / MWAIT than upstream.
    }
