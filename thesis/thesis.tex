...
In this chapter, we present our solution to reduce i-cache misses and branch-misprediction in server applications with minimal manual changes required to the their source code.
We explicitly target multi-processor systems, addressing the shortcomings of the 2000s-era solutions in this field (see Section~\ref{ch:relwork:anasmp}).
We avoid the design-fallacies of previous user-space solutions and propose an operating-system-based approach implemented in the OSv library OS.%, allowing for clean modeling of our design in  a modern C++ codebase.

% appdev perspective: stage api, must parition the code between stages (see STEPS) + stack-based  API with lock-guards, simplifying adoption
% implementation then allocates cores to stages depending on 'load' of the stage and tracks which stage a thread is currently in
% when thread switches stages, the allocation policy is evaluated and the least-loaded core (run-queue length) is chosen as migration target
% when a thread blocks, it is simply removed from the core's local ready queue. when woken up again, the allocation policy is evaluated and the threadd migrated to the least-loaded core
% the promise is the same as in computation spreading and the proof-of-concept: pre-warmed caches.
% the obvious trade-off: thread migratino cost vs advantage of warm caches

From an application developer's perspective, our solution provides a system API allowing to define \emph{stages} as objects and switch between them through function calls.
Stages distinguish themselves through ideally disjoint instruction working sets where each on its own fits into the private cache of the CPU cores.
The application developer inserts stage switching calls into the request-handling code path, thus partitioning its total instruction working set into stages.
For example, a request-handler will
\begin{enumerate*}[label=(\alph*)]
    \item read the request data from a socket, using the network stack,
    \item deserialze \& parse it,
    \item  perform the requested operation,\label{en:di:ov:processing}
    \item serialize the response and
    \item send it back to  the client.
\end{enumerate*}
While not true for all types of servers, database systems for example exhibit a a significant i-cache footprint in step~\ref{en:di:ov:processing} \emph{per high-level SQL operation} (see Section~\ref{ch:relwork:steps}).
...
Before calling \texttt{read} on the network socket, the developer would then switch to the network-stack-stage.

On the system side, we track the association of threads and their current stage in the thread's TCB and per stage the number of threads currently associated with it.
We use this data to allocate the available CPU cores proportionally among the defined stages, thus dedicating an integer number of cores to each stage.
The system follows the following guiding principle: \textbf{threads are only dispatched on cores that are allocated to the thread's current stage}.
...
The other event is where a blocked thread is woken up: on block, a thread is removed from the CPU's runqueue but its stage association is left untouched.
When the thread is later woken up again, the \emph{waker} evaluates the core allocation policy and enqueues the still-sleeping thread into the target CPU's runqueue.
The scheduling policy empoloyed per core is non-preemptive round-robin. \todo{we should actually try preemptive}

\begin{figure}[h]
    \missingfigure[width=0.8\textwidth]{visualize the solution}
    \caption{TODO}
\end{figure}
...
by dedicating CPU cores and hence their private caches to a stage and scheduling threads exclusively on the cores of their current stages, the i-cache always contains the instruction working set of all threads in the CPU's runqueue.
...
We conclude with a discussion of the trade-offs made in our design in Section~\ref{ch:di:discuss}.
