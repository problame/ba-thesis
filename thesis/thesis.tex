...
and present an application of these proposals to a research database management system, resulting in significant performance benefits due to reduced i-cache misses and better branch prediction results.

Motivated by recent profiling work of scale-out applications, proving that their instruction cache footprint is cause for major performance bottlenecks on contemporary CPUs,
we investigate the stagnant adoption of above techniques in today's production software systems and find assumptions made in the early 2000s' designs that no longer hold on today's SMP systems and memory hierarchies.

We finish this chapter with the starting point for this thesis:
given the \emph{software data spreading} technique which spreads the \emph{data} cache footprint of an application over multiple cores,
we present a proof-of-concept implementation at the KIT OS group, which applies the idea of software data spreading to the \emph{instruction} cache footprint of an application.

\section{Concurrency Models in the Early 2000s}\label{ch:relwork:conmod}
...
We start our investigation of staged execution with \emph{cohort scheduling}, dating back to the early 2000s:
...
The size of a cohort represents the central tradeoff in the scheduling policy: large cohorts yield fewer amortized cache misses but cause higher response time due to progress only being made once enough threads have reached the synchronisation point required for batch dispatch.~\cite{cohort}
...
we imagine 3 \todo{num} migration points $A$, $B$ and $C$ and a team of 10 threads executing between $A$ and $B$, operating on a full, warm i-cache.
...
\section{Profiling Datacenter Applications}
...
While the micro-architectural performance problems caused by large i-cache footprints were already observed by \cite{cohort} and \cite{steps} in the early 2000s,
...
In particular, the authors find that more than 5\% of cycles starved on empty front-end buffers, which is attributed to i-cache misses in the on-core L2 cache, resulting in slow off-core accesses to the CPU's last level cache.
Additionally, the instruction working set of applications is shown to grow significantly over their active develpoment time, worsening the situation.%
\footnote{\cite{kanev2015profiling} shows examplary results of up to 27\% per year}
The authors do not provide explicit reasons for the abnormal size of the instruction working sets, only mentioning \textquote{lukewarm} code and static linking as possible contributors.~\cite{kanev2015profiling}
...
% see related work cohort scheduling
...
% paper 'duality of operating system structures', das sagt, dass message passing (also im prinzip staged computation) und mulit-threading im prinzip ds selbe ist
% => beweis, dass stages message passing sind: stagedExecutionDBMS, 4.1 rechts unten: individual servers
% stagedExecutionDBMS p6 Maintainability
...
\todo{finish this...}
\todo{affinity scheudling paper? just ignore it?}
...
% TODO compare I-cache missesa and mispred branches with Steps, who claim that, see steps, p2 rechts oben => 30 warehouses!
