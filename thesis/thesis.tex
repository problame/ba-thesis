...
When quoting benchmarks on Linux, we refer to measurements conducted in a virtual machine running the official MySQL 5.6.38 Docker image.
Inside the VM, we bind-mount the MySQL data directory from the ext4-formatted root partition on the emulated HDD.
With regards to the file system, this gives the Linux VM a slight advandtage over our solution and upstream OSv because ext4 has generally lower overhead. \todo{proof}
For all other aspects, the QEMU configuration described OSv applies.
...
Therefore, we re-use a one-time-generated pre-seed with two. \todo{mention weights?}
...
On our host system, this is \texttt{HZ=1000}.~\cite{perfTimeMultiplexing}
\todo{change that?}
...
We process the result data using Python, depending on the pandas 0.22 package in combination with numpy 1.14.0 and matplotlib 2.1.2.
All graphs and figures in this chapter are rendered reproducibly from the raw data.
All scripts used for this process as well as the raw data can be found in the appendix.
\todo{check that + appendix}
...
55\% of pipeline slots during a TPC-C benchmark run are frontend-bound, 20\% backend-bound and 5\% lost to bad speculation, leaving merely 20\% of pipeline slots to be actually retired. % topdown_comparison.png, Docker
Looking at L2 cache misses, we observe more than 66\% of private L2-cache misses are due to code reads.
\todo{intro to top-down?, proof? i measured that...}
...
the existing MySQL code-base is large and thus not trivially refactorable to the application architectures presented in Chapter~\ref{ch:relwork},
and the threading model is compatible with our solution.
The L2 metrics further support the assumption that MySQL's can benefit from spreading its instruction working set over multiple cores.
...
A human then manually inserts the switching calls directly before the function call.\todo{cite}

For this thesis, we used the above approach to find an optimal partitioning for 3 stages.
A larger number of stages was impractical due to different schedules and the runtime of above algorithm.
