...
For example, when choosing 10 instead of 1000Hz as the PAS frequency, each invocation of the PAS can in theory run up to 100 times longer, but since the entire page cache may be scannable in a shorter time, the PAS may go back to sleep early.
...
The PAS thread's instruction working set is disjoint from the stage threads' working set and given that the latter is chosen to just fit the CPU-private cache, the frequent scheduling of the PAS thread causes thrashing of the private cache of cpu0.
