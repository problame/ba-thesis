...
Network servers are of every-increasing importance in todays cloud-based distributed infrastructure, handling requests from clients at high degrees of concurrency.
...
Given the inceasing cpu-memory-performance gap, it is paramount that the request handler's working set fits into the private CPU caches.
The instruction working set if of particular importance to feed the execution pipeline of contemporary superscaler out-of-order processors.
...
Howver, recent large-scale profiling at Google suggests that this memory hierarchy fails to meet the demands of contemporary server applications:
request handling code exhibits little spacial locality and thrashes the private CPU caches, leading to suboptimal branch prediction and pipeline stalls.\cite{kanev2015profiling}
